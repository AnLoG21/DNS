...
def parse_characteristics_page(driver, url):
    ...
    cat = soup.find('span', class_="app-catalog-nhng7j e1l49rf80")
    name = soup.find('h1', class_="e1ubbx7u0 eml1k9j0 app-catalog-lc5se5 e1gjr6xo0")
    price = soup.find('div', class_="app-catalog-0 e1xrzi9t0")
    #desc = soup.find('div', class_="app-catalog-u6hio8 e9hu13w0")
    #avail = soup.find('p', class_="app-catalog-1l95nvm ey6lrvn0")
    ...
    for i in span_tags:
        if bool(str(i).find('class="app-catalog-nhng7j e1l49rf80""') != -1):
            category = i
...
def get_all_category_page_urls(driver, url_to_parse):
    ...
    span_tags = soup.find_all('span')
    for i in span_tags:
        if bool(str(i).find('data-meta-name="SubcategoryPageTitle__product-count"') != -1):
...
def get_urls_from_page(driver):
    ...
    return list(map(
        lambda element: 'https://www.citilink.ru/' + element.get("href") + 'properties/',
        elements
    ))
...
def to_db(data, driver, file_name="db"):
    ...
    notebook["Категория"] = cat.text.lstrip(': ')
    notebook["Наименование"] = name.text[15:]
    notebook["Цена"] = int(price.text.replace(' ', '')[:-1])
    ...
    query = """ CREATE TABLE IF NOT EXISTS citilink(id INTEGER, price INT, name TEXT, category TEXT) """
    cursor.execute(query)
    # добавляем строку в таблицу people
    query1 = """INSERT INTO citilink(price, name, category) VALUES (%s, %s, %s)"""
    cursor.execute(query1,
                    ((int(price.text.replace(' ', '')[:-1])), str(name.text[15:]), str(cat.text.lstrip(': '))))
